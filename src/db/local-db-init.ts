...
const DB_USER_WRITER = requireEnv('DB_USER_WRITER');
...
async function waitForDb(): Promise<void> {
    ...
    for (let i = 0; i < 30; i++) {
        try {
            execSync(
                `docker exec deputedex-db pg_isready -U ${DB_USER} -d ${DB_NAME}`,
                { stdio: 'ignore' }
            );
...
(async () => {
    try {
        ...
        execSync('docker compose --env-file .env.local up -d', {
            stdio: 'inherit',
        });
        ...
        const sql = `
            ...
            BEGIN
               IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = '${DB_USER_WRITER}') THEN
                  CREATE ROLE ${DB_USER_WRITER} LOGIN PASSWORD '${DB_PASSWORD_WRITER}';
               END IF;
        ...
        execSync(
            `docker exec -i deputedex-db psql -U ${DB_USER} -d ${DB_NAME}`,
            {
                input: sql,
