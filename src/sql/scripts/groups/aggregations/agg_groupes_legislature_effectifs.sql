...
WITH legislatures_ref AS (
    ...
    FROM param_legislatures pl
     ...
     effectifs_photo AS (
         ...
         GROUP BY ag.groupe_id, ag.groupe_legislature
     ),
     effectifs_historiques AS (
         SELECT
             ag.groupe_id,
             ag.groupe_legislature AS legislature,
             COUNT(DISTINCT ag.acteur_uid) AS nb_acteurs_historiques
         FROM acteurs_groupes ag
         GROUP BY ag.groupe_id, ag.groupe_legislature
     )
SELECT
    ...
    COALESCE(eh.nb_acteurs_historiques, 0) AS nb_acteurs_historiques
FROM ref_groupes rg
