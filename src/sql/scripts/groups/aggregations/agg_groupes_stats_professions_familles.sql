-- OK valide
create materialized view agg_groupes_stats_professions_familles as
with legislatures_ref as (select pl.number as legislature,
     ...
     acteurs_dedup as (select a.uid,
                              case
                                  when a.profession_famille in (
                                                                'Artisans, commerçants, chefs d''entreprises',
                                                                'Artisans, commerçants et chefs d''entreprise'
                                      ) then 'Artisans, commerçants et chefs d''entreprise'
                                  else coalesce(a.profession_famille, 'Non renseignée')
                                  end as profession_famille_normalisee,
                              row_number() over (
                                  partition by a.uid
     ...
     base as (select m.legislature,
                     ...
                     coalesce(ad.profession_famille_normalisee, 'Non renseignée') as profession_famille,
                     m.acteur_uid
...
create unique index on agg_groupes_stats_professions_familles (
                                                               groupe_id,
                                                               legislature,
                                                               profession_famille
    );